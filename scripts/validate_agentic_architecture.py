#!/usr/bin/env python3
"""Validate Agentic Decision Intelligence architecture invariants."""
...
def require_markers(root: Path, rel: Path, code: str, markers: tuple[str, ...], problems: list[Problem]) -> None:
    text = read(root, rel)
    if not text:
...
def check_capability_levels(root: Path, problems: list[Problem]) -> None:
    ...
    rows = capability_rows(read(root, rel))
    if set(rows) != set(EXPECTED_LEVELS):
        problems.append(Problem("AI002", str(rel), f"AI level table must cover exactly M00..M11; found {sorted(rows)}"))
        return
    for mission, expected in EXPECTED_LEVELS.items():
        if not level_matches(mission, rows[mission]):
            problems.append(Problem("AI002", str(rel), f"{mission} AI level must be {expected}; found {rows[mission]}"))


def check_decision_contract(root: Path, problems: list[Problem]) -> None:
    require_markers(root, Path("docs/DECISION-CONTRACTS.md"), "AI003", (
        "DecisionPacket", "evidence_refs", "confidence", "uncertainty", "missing_evidence",
        "freshness", "expires_at", "risk_level", "policy_decision",
        "ActionIntent **không** đồng nghĩa execution permission", "Execution Record",
    ), problems)


def check_tool_governance(root: Path, problems: list[Problem]) -> None:
    rel = Path("docs/TOOL-REGISTRY-STANDARD.md")
    require_markers(root, rel, "AI004", ("EXTERNAL_SIDE_EFFECT", "ActionIntent", "Policy/Risk", "approval"), problems)
    require_markers(root, rel, "AI005", ("permission:", "risk_ceiling:", "requires_approval:", "audit_fields:"), problems)


def check_untrusted_boundary(root: Path, problems: list[Problem]) -> None:
    require_markers(root, Path("docs/AI-AGENT-DECISION-ARCHITECTURE.md"), "AI006", (
        "MODEL OUTPUT = UNTRUSTED INPUT", "DECISION ≠ EXECUTION", "AI ADVICE ≠ EXECUTION AUTHORITY",
        "POLICY BEFORE CONSEQUENTIAL ACTION",
    ), problems)


def check_evaluation(root: Path, problems: list[Problem]) -> None:
    require_markers(root, Path("docs/AGENT-EVALUATION-STANDARD.md"), "AI007", (
        "task_success", "tool_selection_accuracy", "tool_argument_accuracy", "unsupported_claim_rate",
        "policy_block_accuracy", "confidence_calibration", "decision_latency", "cost_per_decision",
...
def check_freshness(root: Path, problems: list[Problem]) -> None:
    require_markers(root, Path("docs/MCP-2026-OPERATING-NOTES.md"), "AI008", ("**Verified:**", "**Volatility:**", "freshness/implementation note"), problems)
    rel = Path("docs/BOT-ENGINEERING-REFRESH-2026.08.md")
    text = read(root, rel)
    for source_id in ("## EXT:MCP:2026-07-28", "## EXT:AGENT:TOOL-SEARCH-PROGRAMMATIC", "## EXT:AGENT:DURABLE-HITL"):
...
def check_provider_neutrality(root: Path, problems: list[Problem]) -> None:
    require_markers(root, Path("docs/AI-PROVIDER-CAPABILITY-MATRIX.md"), "AI009", (
        "không được phụ thuộc trực tiếp vào một provider SDK", "AI Provider Interface", "Provider Adapter(s)",
...
def check_multi_agent_boundary(root: Path, problems: list[Problem]) -> None:
    require_markers(root, Path("docs/AI-CAPABILITY-LEVELS.md"), "AI010", (
        "A4 — Multi-Agent Optional Advanced", "sau M11", "không phải core Mission",
    ), problems)
    ...
    if not re.search(r"advanced option\s+sau(?:\s+khi)?\s+M11", evolution, re.IGNORECASE):
...
def check_outcome_learning(root: Path, problems: list[Problem]) -> None:
    require_markers(root, Path("docs/DECISION-OUTCOME-MEMORY.md"), "AI011", (
        "Decision\n→ Action\n→ Outcome\n→ Evaluation\n→ Proposed Improvement\n→ Offline Test / Experiment\n→ Review\n→ Deploy",
        "Policy authority không được tự sửa", "không tự rewrite production policy/prompt/weights",
    ), problems)


def check_programmatic_orchestration(root: Path, problems: list[Problem]) -> None:
    require_markers(root, Path("docs/PROGRAMMATIC-TOOL-ORCHESTRATION.md"), "AI012", (
        "READ_ONLY", "Không cho free orchestration", "publish;", "spend;", "account/security change;",
        "destructive delete;", "allowlisted tools",
    ), problems)
