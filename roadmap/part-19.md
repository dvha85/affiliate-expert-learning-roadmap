# Phần 19 — PRODUCTION, SECURITY & AUTOMATION

- Lịch đề xuất: **Tháng 12**
- Quy mô: **5 chương / 32 bài**
- Chỉ tick bài khi đã đạt đủ [5 tiêu chí PASS](../docs/PASS-CRITERIA.md).

## Checklist bài học

### Chương 73 — Production Engineering

- [ ] **73.1** — Logging
- [ ] **73.2** — Metrics
- [ ] **73.3** — Monitoring
- [ ] **73.4** — Health Check
- [ ] **73.5** — Tracing
- [ ] **73.6** — Alerting

### Chương 74 — Reliability

- [ ] **74.1** — Retry
- [ ] **74.2** — Timeout
- [ ] **74.3** — Circuit Breaker
- [ ] **74.4** — Queue
- [ ] **74.5** — Idempotency
- [ ] **74.6** — Disaster Recovery

### Chương 75 — Security

- [ ] **75.1** — API Key
- [ ] **75.2** — Secret Management
- [ ] **75.3** — OAuth
- [ ] **75.4** — Authentication
- [ ] **75.5** — Authorization
- [ ] **75.6** — Encryption
- [ ] **75.7** — Audit Log

### Chương 76 — Automation Governance

- [ ] **76.1** — Ranh giới automation hợp lệ
- [ ] **76.2** — Cấm fake traffic, engagement, review và order
- [ ] **76.3** — Cấm spam, platform manipulation và policy evasion
- [ ] **76.4** — Automation cho collection, analysis và reporting
- [ ] **76.5** — Automation cho ranking, alert, draft và experiment analysis
- [ ] **76.6** — Approval, audit và kill switch

### Chương 77 — Deployment

- [ ] **77.1** — Docker
- [ ] **77.2** — Environment
- [ ] **77.3** — CI/CD
- [ ] **77.4** — Database Migration
- [ ] **77.5** — Cloud
- [ ] **77.6** — Backup
- [ ] **77.7** — Cost Monitoring

## Cổng thực hành

- [ ] **PROJECT 13 — Production Affiliate Bot**
- [ ] Có artifact/evidence được lưu trong repo hoặc liên kết từ Issue
- [ ] Viết retrospective: kết quả, sai lệch, điều học được, bước tiếp theo

## Hoàn thành phần

- [ ] Tất cả bài học đã PASS
- [ ] Project/Lab/Pass Gate (nếu có) đã hoàn tất
- [ ] Knowledge Base đã cập nhật
- [ ] Đã chọn bài đầu tiên của phần tiếp theo

[← Roadmap tổng](../ROADMAP.md)
